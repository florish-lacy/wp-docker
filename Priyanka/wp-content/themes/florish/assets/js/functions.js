    ...
    success: function (response) {  
      ...
      var data = JSON.parse(response);
       ...
       if(data.error==false){
        jQuery('.response_essage').html('<div class="alert alert-success" role="alert">'+data.message+'</div>');
       // jQuery('#UserRegForm').trigger("reset");
       jQuery('.set_email').text(jQuery('#forgot_email').val());
...
//jQuery("#verify_email").submit(function(event) {
  //e.preventDefault();
  var digit1 = jQuery("#digit-1").val();
  var digit2 = jQuery("#digit-2").val();
  var digit3 = jQuery("#digit-3").val();
  var digit4 = jQuery("#digit-4").val();
  var digit5 = jQuery("#digit-5").val();
  var digit6 = jQuery("#digit-6").val();
