...
<body <?php body_class(); ?>>
	...
	<nav class="fl-navbar navbar navbar-expand-lg" aria-label="Offcanvas navbar large">
		<div class="container-fluid navbar__inner">
			...
			?>
			<a class="navbar-brand d-flex h-100 px-2" href="<?php echo get_site_url(); ?>"><img
					src="<?php echo $logo_src; ?>" alt="Florish logo" /></a>
			...
			<div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasNavbar"
				...
				<div
					class="offcanvas-body d-flex flex-column-reverse justify-content-end flex-lg-row align-items-lg-center gap-3">
					<!-- NAV -->
					<?php
					wp_nav_menu(
						array(
						...
						<div class="user-menu d-flex align-items-center justify-content-center flex-grow-1">
							...
							<?php if (!is_user_logged_in()) { ?>
								<button data-bs-toggle="modal" data-bs-target="#login-popup"
									class="toggle-on-hover nav-link d-block p-2 flex-grow-1 text-center">
							...
							<?php } else { ?>
								...
								?>
								<div class="dropdown">
									<button class="nav-link log-out" role="button" data-bs-toggle="dropdown"
										aria-expanded="false">
										<i class="fa-solid fa-user"></i> <span class="sr-only">Account Settings</span>
									</button>
									<ul class="dropdown-menu dropdown-menu-end gap-1 p-2 rounded-3 mx-0 shadow"
										data-bs-theme="light">
										<li>
											<b>
												My Account
						...
						if (!wc_user_has_role($current_user, 'nursery')) { ?>
							...
							<?php if (!is_user_logged_in()) { ?>
								<div class="cart d-flex align-items-center flex-grow-1">
									<button data-bs-toggle="modal" data-bs-target="#login-popup"
										class="nav-link d-block p-2 flex-grow-1 text-center">
							...
							<?php } else { ?>
								...
								<div class="cart d-flex align-items-center flex-grow-1">
									...
									<a href="<?php echo wc_get_cart_url() ?>" class="position-relative flex-grow-1 text-center nav-link <?php if ($items_count)
										...
										<?php if ($items_count) { ?>
											<span
												class="cart__badge position-absolute top-0 start-100 translate-middle badge rounded-pill bg-primary">
